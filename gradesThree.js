...
function handleAddBtnThree() {
  const tableTbody = document.getElementById("table_tbody3");
  const inputTr = document.getElementById("inputTr3");

  if (inputTr == null) {
    const newTr = document.createElement("tr");
    newTr.setAttribute("id", "inputTr3");

    const emptyTh = document.createElement("th");

    const newTh1 = document.createElement("th");
    const input1 = document.createElement("select");
    input1.setAttribute("id", "type3");
    ...
    input2.setAttribute("id", "essential3");
    ...
    document.getElementById("inputTr3").remove();
  }
}

// ----------------- 저장 버튼 클릭 이벤트 -----------------

function handleSaveBtnThree() {
  const tableTbody = document.getElementById("table_tbody3");

  const type = document.getElementById("type3");
  const essential = document.getElementById("essential3");
  const subname = document.getElementById("subname3");
  const grade = document.getElementById("grade3");
  const attendance = document.getElementById("attendance3");
  const assignment = document.getElementById("assignment3");
  const midterm = document.getElementById("midterm3");
  const final = document.getElementById("final3");
  ...
  if (temp1 >= 0) {
    if (!tableTbody1.rows[temp1].cells[11].innerHTML.includes("F")) {
      return alert(
        "재수강 과목은 1학년 과목 중 F학점인 경우만 입력이 가능합니다."
      );
    }
    if (!tableTbody1.rows[temp1].cells[1].innerHTML.includes(type.value)) {
      return alert("올바른 이수 과목을 선택해 주세요.");
    } else if (
      !tableTbody1.rows[temp1].cells[2].innerHTML.includes(essential.value)
    ) {
      return alert("올바른 필수 과목을 선택해 주세요.");
    } else if (
      !tableTbody1.rows[temp1].cells[4].innerHTML.includes(grade.value)
    ) {
      return alert("올바른 학점을 선택해 주세요.");
    }
  }

  // 2학년 과목 중복 확인 후 성적 F 학점일 경우에만 입력
  const tableTbody2 = document.getElementById("table_tbody2");
  let temp2 = -1;
  for (let i = 0; i < tableTbody2.rows.length; i++) {
    if (tableTbody2.rows[i].cells[3].innerHTML.includes(subname.value)) {
      temp2 = i;
    }
  }
  if (temp2 >= 0) {
    if (!tableTbody2.rows[temp2].cells[11].innerHTML.includes("F")) {
      return alert(
        "재수강 과목은 1학년 과목 중 F학점인 경우만 입력이 가능합니다."
      );
    }
    if (!tableTbody2.rows[temp2].cells[1].innerHTML.includes(type.value)) {
      return alert("올바른 이수 과목을 선택해 주세요.");
    } else if (
      !tableTbody2.rows[temp2].cells[2].innerHTML.includes(essential.value)
    ) {
      return alert("올바른 필수 과목을 선택해 주세요.");
    } else if (
      !tableTbody2.rows[temp].cells[4].innerHTML.includes(grade.value)
    ) {
      return alert("올바른 학점을 선택해 주세요.");
    }
  }
  ...
  deleteInput.innerHTML = `<th><input type="checkbox" class="checkbox-class3"></th>`;
  ...
  const enggrade = engGrade3(intSumGrade);
  ...
  if (somthingSum3("grades_sum3", 4) == 1) {
    document.getElementById("grades_sum3").innerText = "1";
  } else {
    somthingSum3("grades_sum3", 4);
  }
  somthingSum3("grades_sum3", 4);
  somthingSum3("attendance_sum3", 5);
  somthingSum3("assignment_sum3", 6);
  somthingSum3("midterm_sum3", 7);
  somthingSum3("final_sum3", 8);
  somthingSum3("total_sum3", 9);
  ...
  document.getElementById("total_avg3").innerText = avgs_sum;

  const tatal_grade = engGrade3(avgs_sum);
  document.getElementById("total_grade3").innerText = tatal_grade;

  // 정렬
  let table, rows, switching, i, x, y, shouldSwitch;
  table = document.getElementById("table_container3");
  switching = true;
  while (switching) {
    switching = false;
    rows = table.rows;
...
function handleDeleteBtnThree() {
  const tableTbody = document.getElementById("table_tbody3");
  const chkbox = document.querySelectorAll(".checkbox-class3");
  const totalRow = tableTbody.rows.length;
  if (totalRow > 0) {
    let CheckedCnt = 0;
    for (let i in chkbox) {
    ...
    if (totalRow == 1) {
      document.getElementById("grades_sum3").innerText = "";
      document.getElementById("attendance_sum3").innerText = "";
      document.getElementById("assignment_sum3").innerText = "";
      document.getElementById("midterm_sum3").innerText = "";
      document.getElementById("final_sum3").innerText = "";
      document.getElementById("total_sum3").innerText = "";
      document.getElementById("total_avg3").innerText = "";
      document.getElementById("total_grade3").innerText = "";
    } else {
      if (somthingSum3("grades_sum3", 4) == 1) {
        document.getElementById("grades_sum3").innerText = "1";
      } else {
